  ...
  if (is_current && status === "not_done") {
    return (
      <>
        ...
        >
          {is_point && (
            <div className="bg-red-600 w-1 h-1 xl:w-2 xl:h-2 rounded-full absolute top-2/4 left-0 -translate-x-full -translate-y-2/4" />
          )}
...
export default function Road({ data }: { data: roadType }) {
  // variables
  const changeStep = useStepsStore((state) => state.changeStep)
  ...
  return (
    <div className="hidden lg:flex items-center absolute top-2/4 left-2/4 -translate-x-2/4 -translate-y-2/4">
      ...
      {data.finish_icon && (
        <SpecialPoint has_next={false} icon={data.finish_icon} />
      )}
    </div>
  )
}
